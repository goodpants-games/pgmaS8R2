...
function table.pick_weighted_random(t, w, r)
	...
	if #w ~= #t then
		error("table.pick_weighted_random weight and value tables should be the same length")
	end
	local sum = 0
...
function table.compact(t, range)
	...
	elseif type(range) == "number" then
		...
		error("table.compact - range must be a number or table", 2)
	end
	return r
...
if not table.clear then
	...
	if not imported then
		...
		function table.clear(t)
			assert:type(t, "table", "table.clear - t", 1)
			local k = next(t)
			while k ~= nil do
				t[k] = nil
				k = next(t)
...
function table.deep_copy(t)
	assert:type(t, "table", "table.deep_copy - t", 1)
...
function table.shallow_overlay(dest, ...)
	assert:type(dest, "table", "table.shallow_overlay - dest", 1)
	for i = 1, select("#", ...) do
		local t = select(i, ...)
		assert:type(t, "table", "table.shallow_overlay - ...", 1)
		for k, v in pairs(t) do
			dest[k] = v
		end
	end
...
function table.deep_overlay(dest, ...)
	assert:type(dest, "table", "table.deep_overlay - dest", 1)
	for i = 1, select("#", ...) do
		local t = select(i, ...)
		assert:type(t, "table", "table.deep_overlay - ...", 1)
		for k, v in pairs(t) do
...
function table.collapse(t)
	assert:type(t, "table", "table.collapse - t", 1)
	local r = {}
...
function table.extract(t, n)
	assert:type(t, "table", "table.extract - t", 1)
	assert:type(n, "number", "table.extract - n", 1)
	local r = {}
	for i = 1, #t, n do
		r[i] = {}
		for j = 1, n do
